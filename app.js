...
const show = (v)=>{ Object.values(views).forEach(x=>x.classList.add('hidden')); views[v].classList.remove('hidden') }
...
postsList.innerHTML = '';
if(!posts.length){ postsList.appendChild(el('div','muted')).textContent = 'No posts yet'; return }
posts.forEach(p=>{
...
const viewBtn = el('button','small'); viewBtn.textContent = 'View';
viewBtn.onclick = ()=> showDetail(p.id);
...
await fetchPosts(searchInput.value.trim());
}
...
if(id){
await fetch(`${API_BASE}/posts/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
} else {
await fetch(`${API_BASE}/posts`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
}
show('home');
await fetchPosts(searchInput.value.trim());
});
...
// Navigation
qs('#nav-home').addEventListener('click', (e)=>{ e.preventDefault(); show('home'); });
